...
estimated_freq = zeros(1, length(t_input));
...
estimated_freq = NaN(size(phase_angle_deg));

estimated_freq(1) = 0; % or some starting value

for n = 2:length(phase_angle_deg)

delta = phase_angle_deg(n) - phase_angle_deg(n-1);
