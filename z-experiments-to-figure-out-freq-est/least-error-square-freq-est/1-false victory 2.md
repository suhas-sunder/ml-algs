...
phase_angle_rad = zeros(1, length(t_input));
...
current_freq_estimate = estimated_freq(n);

new_phase = 0;

for k = 1:1

matrix_A = generate_filter_matrix(window, t_input(n) - t_input(n-1), current_freq_estimate, fundamental_filter, second_harmonic_filter, third_harmonic_filter, fourth_harmonic_filter, fifth_harmonic_filter, sixth_harmonic_filter, dc_filter);

target_pinv_A = pinv(matrix_A); % ignore very small components
...
phase_angle_rad(n) = atan2(imaginary_part_Vp_cos_theta, real_part_Vp_sin_theta);

end

new_phase = unwrap(atan2(imaginary_part_Vp_cos_theta, real_part_Vp_sin_theta));

% Unwrap the phase before computing delta

unwrapped_phase = unwrap(phase_angle_rad); % Convert to radians and unwrap

delta = new_phase - unwrapped_phase(n-1);

% Frequency estimate: delta_phi / (2π / fs) = delta_phi * fs / 2π

new_freq_estimate = abs(delta) / ((2 * pi)/fs);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

current_freq_estimate = new_freq_estimate;

if(new_freq_estimate > 40 && new_freq_estimate < 70)

if(abs(new_freq_estimate - current_freq_estimate) <= tolerance )

estimated_freq(n:end) = new_freq_estimate;
